...
@logwrap
def loop_find(query, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.5, intervalfunc=None, device=None):
    """
    ...
    while True:
        screen = (device or G.DEVICE).snapshot(filename=None, quality=ST.SNAPSHOT_QUALITY)
...
@logwrap
def try_log_screen(screen=None, quality=None, max_size=None, device=None):
    """
    ...
    if screen is None:
        screen = (device or G.DEVICE).snapshot(quality=quality)
