...
def shell(cmd, device=None):
    ...
    :Example:
        ...
        >>> shell("ls")
    """
    return (device or G.DEVICE).shell(cmd)
...
def start_app(package, activity=None, device=None):
    ...
    """
    (device or G.DEVICE).start_app(package, activity)
...
def stop_app(package, device=None):
    ...
    """
    (device or G.DEVICE).stop_app(package)
...
def clear_app(package, device=None):
    ...
    """
    (device or G.DEVICE).clear_app(package)
...
def install(filepath, device=None, **kwargs):
    ...
    """
    return (device or G.DEVICE).install_app(filepath, **kwargs)
...
def uninstall(package, device=None):
    ...
    """
    return (device or G.DEVICE).uninstall_app(package)


@logwrap
def snapshot(filename=None, msg="", quality=None, max_size=None, device=None):
    """
    ...
    if filename:
        ...
        screen = (device or G.DEVICE).snapshot(filename, quality=quality, max_size=max_size)
...
def home(device=None):
    ...
    :Example:
        >>> home()
    """
    (device or G.DEVICE).home()


@logwrap
def touch(v, times=1, device=None, **kwargs):
    """
    ...
    if isinstance(v, Template):
        pos = loop_find(v, timeout=ST.FIND_TIMEOUT, device=device)
    else:
        try_log_screen()
        pos = v
    for _ in range(times):
        (device or G.DEVICE).touch(pos, **kwargs)
        time.sleep(0.05)
...
def double_click(v, device=None):
    ...
    if isinstance(v, Template):
        ...
        try_log_screen()
        pos = v
    (device or G.DEVICE).double_click(pos)
    delay_after_operation()
    return pos


@logwrap
def swipe(v1, v2=None, vector=None, device=None, **kwargs):
    """
    ...
    if isinstance(v1, Template):
        pos1 = loop_find(v1, timeout=ST.FIND_TIMEOUT, device=device)
    else:
        try_log_screen()
        pos1 = v1
    ...
    if v2:
        if isinstance(v2, Template):
            pos2 = loop_find(v2, timeout=ST.FIND_TIMEOUT_TMP, device=device)
        else:
            pos2 = v2
    elif vector:
        if vector[0] <= 1 and vector[1] <= 1:
            w, h = (device or G.DEVICE).get_current_resolution()
            vector = (int(vector[0] * w), int(vector[1] * h))
        pos2 = (pos1[0] + vector[0], pos1[1] + vector[1])
    else:
        raise Exception("no enough params for swipe")

    (device or G.DEVICE).swipe(pos1, pos2, **kwargs)
    delay_after_operation()
    return pos1, pos2


@logwrap
def pinch(in_or_out='in', center=None, percent=0.5, device=None):
    """
    ...
    """
    try_log_screen()
    (device or G.DEVICE).pinch(in_or_out=in_or_out, center=center, percent=percent)
    delay_after_operation()


@logwrap
def keyevent(keyname, device=None, **kwargs):
    """
    ...
    """
    (device or G.DEVICE).keyevent(keyname, **kwargs)
    delay_after_operation()


@logwrap
def text(text, enter=True, device=None, **kwargs):
    """
    ...
    """
    (device or G.DEVICE).text(text, enter=enter, **kwargs)
    delay_after_operation()
...
@logwrap
def wait(v, timeout=None, interval=0.5, intervalfunc=None, device=None):
    """
    ...
    pos = loop_find(v, timeout=timeout, interval=interval, intervalfunc=intervalfunc, device=device)
    return pos


@logwrap
def exists(v, device=None):
    """
    ...
    """
    try:
        pos = loop_find(v, timeout=ST.FIND_TIMEOUT_TMP, device=device)
...
def find_all(v, device=None):
    ...
    """
    screen = (device or G.DEVICE).snapshot(quality=ST.SNAPSHOT_QUALITY)
...
@logwrap
def assert_exists(v, msg="", device=None):
    """
    ...
    """
    try:
        pos = loop_find(v, timeout=ST.FIND_TIMEOUT, threshold=ST.THRESHOLD_STRICT or v.threshold, device=device)
        return pos
    except TargetNotFoundError:
...
@logwrap
def assert_not_exists(v, msg="", device=None):
    """
